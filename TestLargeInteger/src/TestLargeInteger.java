...
class LargeInteger {
    ...
    public LargeInteger iterativeAddition(LargeInteger other) {
        other.removeLeadingZeros();     
         LargeInteger count=new LargeInteger ( "0" );//creating new LargeInteger of Value Zero
         LargeInteger summer=new LargeInteger ( "1" );//creating a LargeInteger to increase value of counter by 1.
         
         LargeInteger adding=new LargeInteger ( "0" ); //creating a LargeInteger that stores the solution.
        while (!(count.equals(other))){ //creating loop for the sum
        count=count.add(summer);//updating counter
        adding=adding.add(this);//iteratively adding the number, to compute the multiplication.
        }
        return adding.removeLeadingZeros(); 
  ...
  for (int i=n-1; i>=0; i--){ //creating first loop, from n-1 downto 0.
    int carry=0;
    byte[] tmpAdd= new byte [k+1];
    	for (int j=k-1; j>=0;j--){//creating second loop
    	
    		int c= carry + this.digits[j]*other.digits[i];//computing multiplication, and carry: the value over 9 that carries.
    		tmpAdd[j+1]=(byte)((byte)c % 10);//updating the value of tmpAdd according to the multiplication given.
    		carry=c/10;//updating the value of carry.
    			}			
  
    tmpAdd[0]=(byte)carry;//updating value of remainder to tmpAdd
    LargeInteger sum;
    sum=new LargeInteger (tmpAdd, 0,tmpAdd.length);//creating LargeInteger Variable that takes Byte[] tmpAdd as its value.
    
    sum=sum.shiftLeft(n-i-1);//giving "sum" the right place value according to the value of i.
    sum=sum.removeLeadingZeros();
    
    total=(total.add(sum)); //updating the total.
    
        }
      
     return total.removeLeadingZeros();
    ...
    public LargeInteger recursiveFastMultiplication(LargeInteger other) {
        ...
        LargeInteger leftThis, rightThis, leftOther, rightOther;
        LargeInteger term1,  term2,  term3,sum1,sum2,sum3; // creating temporary terms 
        int k = digits.length;
        int n = other.digits.length;
		if (n<k) {//creating an if statement in order to re-do the program if n<k, because of the subtract function (it assumes that n>k)
			LargeInteger a= new LargeInteger("0");
			a=other.recursiveFastMultiplication ( this );
			return a;
		}
		
        if ( k==1) {/*This is used to compute the most basic multiplication procedure when k=1. This happens when recursiveFastMultiplication
        				is called within its own method, which will happen multiple times until k=1 */
        	LargeInteger a= new LargeInteger("0");
        	a= this.standardMultiplication ( other);
            return a;
        }
        ...
        leftOther = new LargeInteger( other.digits, 0, n - n/2 );
        rightOther = new LargeInteger( other.digits, n - n/2, n );
		//setting value of right and left half's of the two LargeIntegers. 
       
		term1= rightOther.recursiveFastMultiplication(rightThis);
	
		term2=leftOther.recursiveFastMultiplication(leftThis);

		sum1 = leftThis.add( rightThis );
		
		sum2= (leftOther.shiftLeft( n/2 -k/2  )).add(rightOther);
		sum3=(term2.shiftLeft( n/2 -k/2 )).add(term1);
		
		term3=sum1.recursiveFastMultiplication(sum2).subtract(sum3); 	//Computing the multiplication according to its equation.
		
			LargeInteger total= new LargeInteger ("0");
				total=term2.shiftLeft( k/2 + n/2);
				
				total=total.add(term3.shiftLeft(k/2));
				
				total=total.add(term1); //adding the terms accordingly in order to create the total.
...
public class TestLargeInteger {
    public static void main( String args[] ) {
        ...
        /*
            ...
            LargeInteger b = new LargeInteger ("891");
           // a=a.getRandom(2);
           // b=b.getRandom(2);
            System.out.println(a + " + " + b + " = " + a.add( b ) );
            System.out.println(b + " - " + a + " = " + b.subtract( a ) );
            System.out.println("This is the correct answer "+ b + " * " + a + " = " + b.recursiveMultiplication( a ) );
            b.recursiveMultiplication( a );
            b.iterativeAddition(a);
            b.standardMultiplication( a );
            System.out.println(b + " * " + a + " st= " + b.standardMultiplication( a ) );
            System.out.println("-----------------------------------------" );
            ...
            System.out.println(currentTime/5);*/
        System.out.println("Comp 250");
